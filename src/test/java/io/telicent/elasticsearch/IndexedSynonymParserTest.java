...
public class IndexedSynonymParserTest {
	...
	public void setup() throws ElasticsearchException, IOException {
		...
		container = new ElasticsearchContainer("docker.elastic.co/elasticsearch/elasticsearch:" + version);
		container.start();
	...
	private void indexSynonyms() throws ElasticsearchException, IOException {
		...
		RestClient restClient = RestClient.builder(new HttpHost(container.getHost(), container.getFirstMappedPort()))
				.build();
	...
	public void loadSynonyms() throws IOException, ParseException {
		...
		IndexedSynonymParser parser = new IndexedSynonymParser(container.getHost(),
				container.getFirstMappedPort().intValue(), INDEXNAME, fieldName, true, true, true, standardAnalyzer);
		parser.parse();
